  ...
  function openDb() {
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = function(event) {
        ...
        db.createObjectStore(TREE_STORE_NAME, { keyPath: ["projectId", "path"] }); 
...
// Ambil semua node/folder/file untuk 1 project tertentu
async function getProjectTree(projectId) {
  const db = await openDb();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(TREE_STORE_NAME, "readonly");
    const store = tx.objectStore(TREE_STORE_NAME);
...
// Tambahkan node ke tree milik 1 project
async function addToProjectTree(projectId, item) {
  const db = await openDb();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(TREE_STORE_NAME, "readwrite");
    const store = tx.objectStore(TREE_STORE_NAME);
    const data = { projectId, ...item };
  ...
  class ixScrollManager {
    ...
    lock() {
      ...
      if (this.mode === 'both' || this.mode === 'desktop') {
        this.target.addEventListener('wheel', this.handlers.wheel, { passive: false });
      }

      if (this.mode === 'both' || this.mode === 'mobile') {
        this.target.addEventListener('touchmove', this.handlers.touchmove, { passive: false });
      }
    }
   ...
   * @param {Object} param0 
   */
  function renderFileContent({ name, type, content, path }) {
    ...
    if (
      ...
      const decoded = (content instanceof Uint8Array || content instanceof ArrayBuffer)
...
editor.setOptions({
 "wrap" : true,
  ...
  enableSnippets: true // Mengaktifkan fitur snippets
   ...
   editorIframe.srcdoc = `
      ...
      <html><head><style>
        ...
        video, img { max-width:300px; height:auto; aspect-ratio:16/9; }
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(TAB_STORE_NAME)) {
          db.createObjectStore(TAB_STORE_NAME, { keyPath: "path" });
        }
      };
      request.onsuccess = () => resolve(request.result);
      request.onerror = () => reject(request.error);
  ...
  document.dispatchEvent(new CustomEvent("tab-closed", { detail: { path } }));
  };
  ...
  span.addEventListener('click', (e) => {
    ...
    document.querySelectorAll('.nav-scroller span[data-path]')
      .forEach(el => el.removeAttribute('data-active'));
    span.setAttribute('data-active', 'true');
    
    renderFileContent({ name, content, path });
    
    document.dispatchEvent(new CustomEvent("tab-opened", {
  ...
  span.addEventListener("dragstart", (e) => {
    ...
    IcodexEditor.plugins.vibrate(25);
  ...
  // Simpan ke IndexedDB
  await saveTabToDb({ path, name, content });
  openTabs.push({ path, name, content }); // Tambahkan ke cache
  ...
  */
    ...
    request.onsuccess = async (event) => {
      ...
      if (cursor) {
        ...
        if (tab.path.startsWith(oldPath + "/")) {
          ...
          const updatedTab = { ...tab, path: updatedPath };
  ...
  document.addEventListener("project-updated", async (e) => {
    ...
    for (const tab of allTabs) {
      const { path: oldPath, name: oldName, content } = tab;
  ...
  function findUpdatedPathInProject(project, oldPath) {
    ...
    /${node.name}` : node.name;
      if (node.type === "file" && fullPath === oldPath) {
        result = { path: fullPath, name: node.name };
        return;
      }
  ...
  window.vanilaJs = {};
  vanilaJs.drawer = {};
  vanilaJs.treeView = class treeView {
  ...
  #createDirectoryNode(node) {
    ...
    renderDropdown._group.push({
      label: 'group-2',
      option: [
        { value: "Copy", icon: "bi bi-copy", id: "copy-folder" },
        { value: "Cut", icon: "bi bi-scissors", id: "cut-folder" },
        { value: "Rename", icon: "bi bi-pencil", id: "rename-folder" },
        { value: "Copy Path", icon: "bi bi-copy", id: "copy-path-folder" }
      ]
    },
    // Group 3: Export & Delete
    {
      label: 'group-3',
      option: [
        { value: "Download", icon: "bi bi-download", id: "export-folder" },
        { value: "Delete", icon: "codicon codicon-trash", id: "delete-this-folder" }
      ]
    });
  ...
  // Setelah valid, akan memanggil this.#handleAddItem untuk menambah data
   #setupDirectoryDropdown(node, dropdownInstance) {
    ...
    dropdownInstance.addEventListener('optclick', (e) => {
      ...
      if (clicked.id === "new-file") {
        ...
        const validateFileName = (value) => {
          ...
          const typeMap = {
    // Web & Markup
    html: 'html', htm: 'htm', css: 'css', js: 'js', jsx: 'jsx', ts: 'ts', tsx: 'tsx',
    json: 'json', xml: 'xml', yml: 'yml', yaml: 'yaml', md: 'md',

    // Text & Config
    txt: 'txt', ini: 'ini', env: 'env', cfg: 'cfg', conf: 'conf', log: 'log',
    gitignore: 'gitignore', 'gitattributes': 'gitattributes', editorconfig: 'editorconfig',

    // Backend Languages
    py: 'py', rb: 'rb', php: 'php', java: 'java', c: 'c', h: 'h',
    cpp: 'cpp', hpp: 'hpp', cs: 'cs', go: 'go', rs: 'rs', swift: 'swift',
    sh: 'sh', bash: 'bash', zsh: 'zsh', pl: 'pl', lua: 'lua',

    // Data & Documents
    csv: 'csv', tsv: 'tsv', xls: 'xls', xlsx: 'xlsx', doc: 'doc', docx: 'docx',
    pdf: 'pdf', rtf: 'rtf', jar: 'jar',

    // Images & Media
    png: 'png', jpg: 'jpg', jpeg: 'jpeg', gif: 'gif', svg: 'svg',
    webp: 'webp', ico: 'ico', bmp: 'bmp', mp3: 'mp3', wav: 'wav',
    ...
    // Archives
    zip: 'zip', rar: 'rar', tar: 'tar', gz: 'gz', tgz: 'tgz', '7z': '7z',

    // Misc
    sql: 'sql', db: 'db', lock: 'lock', bak: 'bak',
    map: 'map', license: 'license',

    // Framework/Tools Specific
    vue: 'vue', svelte: 'svelte', astro: 'astro', dockerfile: 'dockerfile',
    toml: 'toml', makefile: 'makefile', gradle: 'gradle',
    babelrc: 'babelrc', eslintrc: 'eslintrc',
  };
          ...
          const duplicate = (node.children || []).some(child =>
            child.type === "file" && child.name === fileNameOnly
          );
  ...
  if (event.key === 'Enter' || event.key === 13) {
    ...
    const validateFolderName = (value) => {
      ...
      const duplicate = (parentNode?.children || []).some(child =>
    ...
    // Temukan root project untuk referensi pencarian path
    const rootProject = self.#findRootProject(self.data, node);
    ...
    // Hitung parent path
    let parentPath = self._findPathInProjectTree(rootProject, parentNode);

    // Default ke root jika tidak ketemu
    if (!parentPath || parentPath === "null") parentPath = "/";

    // Bentuk path baru (pastikan selalu diawali "/")
    // Pastikan parentPath selalu diawali slash
  ...
  self.#refreshAndSave(rootProject);
    ...
    console.log(`[TreeView] Folder di-rename dari "${oldName}" menjadi "${newName}", path baru: ${newPath}`);

    // Rekursif update path anak-anak
    const updateChildPaths = (folderNode) => {
    if (!folderNode.children) return;

    // Normalisasi path folderNode
    if (!folderNode.path.startsWith("/")) {
      folderNode.path = `/${folderNode.path}`;
    }
    ...
    // Dispatch event untuk TabManager (sinkronisasi tab editor jika perlu)
    const folderRenamedEvent = new CustomEvent("folder-renamed", {
      detail: {
        oldPath,
        newPath,
        renamedNode: node
  ...
  console.log(folderRenamedEvent.detail);
    
    document.dispatchEvent(folderRenamedEvent);

    // Simpan dan refresh TreeView
    self.#refreshAndSave(rootProject);

    // Tutup dialog dan bersihkan
...
.progress-value {
 ...
 `;
      ...
      for (const file of files) {
        await new Promise((resolve, reject) => {
          ...
          reader.onload = () => {
            ...
            const path = this._findPathInProjectTree(rootProject, node);
            const fileNode = {
              type: "file",
              name: file.name,
              mime: file.type,
            ...
            if (!(node.children || []).some(child => child.name === file.name && child.type === "file")) {
    ...
    // ========== COPY PATH FOLDER ===========
      else if (clicked.id === "copy-path-folder") ;    
    ...
    document.dispatchEvent(new CustomEvent("folder-removed", {
    detail: { removedPath: node.path }
  }));
    ...
    // ============ EXPORT ===============
    else if (clicked.id === "export-folder") {
    const rootNode = node;
    
    IcodexEditor.plugins.zip.exportTreeToZip(rootNode, `${rootNode.name}.zip`);
  /*
    // Ambil root node
    const rootNode = node; // bisa node atau this.#findRootProject(...)
    ...
    zip.generateAsync({ type: "blob" }).then(content => {
      ...
      a.download = `${rootNode.name || "project"}.zip`;
      a.click();
  ...
  fileSpan.addEventListener('click',  () => {
    ...
    addTabFile(node.path, node.name, node.content);
  });
  ...
  */
  ...
  #setupFileDropdown(node, fileSpan) {
    ...
    icodeXFileDropdown._group[0].option = [
      { value: "Copy", icon: "bi bi-copy", id: "copy-file" },
      { value: "Cut", icon: "bi bi-scissors", id: "cut-file" },
      { value: "Rename", icon: "bi bi-pencil", id: "rename-file" },
      { value: "Copy Path", icon: "bi bi-copy", id: "copy-path-file" }
    ];
    
    icodeXFileDropdown._group.push({
      label: "group-2",
      option: [
        { value: "Delete", icon: "codicon codicon-trash", id: "delete-this-file" }
      ]
    });
  ...
  document.querySelector('#explorer').addEventListener('scroll', (e) => {
  ...
  !
   ...
   icodeXFileDropdown.addEventListener('optclick', (e) => {
    ...
    // Cek duplikat file dengan nama yang sama di folder yang sama
    ...
    // Hitung ulang path
    const parentPath = self._findPathInProjectTree(rootProject, parentNode);
    ...
    // Broadcast event untuk TabManager
    ...
    console.log(`File berhasil di-rename dari ${oldPath} → ${newPath}`);
    ...
    else if (clicked.id === "copy-path-file") {
      // Copy full path file ke clipboard
      ///App/components/TreeView.js
      // MyProject/src/components/Homepage.jsx
      const rootProject = self.#findRootProject(self.data[0].children, node);
      ...
      const fullPath = self._findPathInProjectTree(rootProject, node);
    
      if (fullPath) {
        navigator.clipboard.writeText(fullPath);
        
        console.warn("Path disalin:\n" + fullPath);
      } else {
        console.error("Gagal menemukan path file!");
      }
    ...
    const event = new CustomEvent("file-removed", {
      detail: { path: node.path }  // path penting untuk identifikasi tabs
    });
  ...
  async #handleAddItem(type, name, node, content = "") {
    ...
    const pathParts = name.trim().split('/');
    ...
    const parentPath = this._findPathInProjectTree(rootProject, node);

    // Pastikan parentPath selalu diawali slash
    const cleanParentPath = parentPath.startsWith("/") ? parentPath : `/${parentPath}`;

    // Bangun path absolut dan bersihkan double slash
    const fullPath = `${cleanParentPath}/${name.trim()}`.replace(/\/+/g, "/");
  ...
  async #createNestedDirectory(path, rootNode) {
    ...
    for (const part of parts) {
      ...
      // Cari folder dengan nama sama dan jenis directory
      let existing = current.children.find(child =>
        child.name === part && child.type === "directory"
      );

      if (!existing) {
        // Temukan path parent dari node saat ini
        const parentPath = this._findPathInProjectTree(this.#findRootProject(this.data, rootNode), current);

        // Pastikan parentPath diawali dengan '/'
        const cleanParentPath = parentPath.startsWith("/") ? parentPath : `/${parentPath}`;

        // Gabungkan path baru dan pastikan tidak ada double slash
        const fullPath = `${cleanParentPath}/${part}`.replace(/\/+/g, "/");
    ...
    // Simpan ke database hanya setelah semua selesai dibuat
  ...
  async #deleteItem(node, targetName, targetKind) {
    if (!node || !node.children) return;

    const confirm = window.confirm(`Yakin ingin menghapus ${targetKind} "${targetName}"?`);
    if (!confirm) return;

    // Cari dan hapus berdasarkan nama dan jenis (file / directory)
    const index = node.children.findIndex(child => child.name === targetName && child.type === targetKind);
    if (index === -1) {
      alert(`${targetKind} "${targetName}" tidak ditemukan.`);
      return;
    }

    node.children.splice(index, 1); // Hapus
  ...
  #handleImportedFileFromWorker(data, node, rootProject) {
    const { name, buffer } = data;
    const fileNode = {
      type: "file",
      name,
      content: new Int8Array(buffer),
      path: `${this._findPathInProjectTree(rootProject, node)}/${name}`
    };

    if (!(node.children || []).some(child => child.name === name && child.type === "file")) {
      node.children = node.children || [];
      node.children.push(fileNode);
    }
  }
  ...
  const vibrate = function(pattern = 50) {
...
class Drawer {
  // Class private untuk nama class CSS container drawer
  #drawerClass = "file_exploler-drawer";


  #newProjectButton = new Button('btn', `
  ...
  // TreeView awal kosong
  #treeView = new TreeView([]);

  constructor() {
    // Buat container utama drawer
    ...
    // Inisialisasi struktur DOM, dialog, dan data awal
    this.#setupDrawerStructure();
    this.#setupProjectDialog();
    this.#refreshTreeViewFromDb();
  }

  // Buat struktur awal drawer di DOM
  #setupDrawerStructure() {
    ...
    // Masukkan tombol "Add Project"
    drawerToolBar.appendChild(this.#newProjectButton.button);

    // Gabungkan ke drawer header
    drawerHeader.appendChild(drawerToolBar);

    // Tambahkan header dan treeView (kosong) ke container
    this.drawerContainer.appendChild(drawerHeader);
    this.drawerContainer.appendChild(this.#treeView.renderTree());

    // Nonaktifkan translasi drawer (untuk animasi/UX?)
    this.drawerContainer.translate = false;
  }

  // Setup dialog input project baru
  #setupProjectDialog() {
    const projectDialog = new CustomDialog({
      title: "Project Baru",
      placeholder: "Masukan Nama Project"
    }, dialogWrap); // `dialogWrap` diasumsikan global

    // Klik tombol add -> tampilkan dialog
    this.#newProjectButton.setOnclickListener(() => {
      projectDialog.show(true);
    });

    // Ketika tombol OK di dialog diklik
    projectDialog.buttonPositive.button.addEventListener("click", async () => {
      const projectName = projectDialog.input.value.trim();
      if (!projectName) return;

      // Struktur project yang akan disimpan
  ...
  async #refreshTreeViewFromDb() {
    ...
    worker.onmessage = (event) => {
      if (event.data.type === "log") {
        ...
        // Buat ulang treeView dengan data dari worker
        this.#treeView = new TreeView(finalTreeData);

        // Tampilkan hasil render ke drawer
        this.drawerContainer.appendChild(this.#treeView.renderTree());
    ...
    worker.onerror = (err) => {
      ...
      // Jika error, tetap tampilkan treeView dari project mentah
...
#openProject(projectId) {
  const drawer = new Drawer(projectId);
   // bersihkan halaman home
   const app = document.getElementById('app')
   if (drawer.drawerContainer.isConnected === false) {
   app.appendChild(drawer.drawerContainer); // tampilkan Drawer
}
  ...
  getAllProjects().then((projects) => {
    this.#ulList.innerHTML = ''; // Bersihkan dulu
    ...
    projects.forEach((p) => {
      ...
      this.addItem(p.name, date, p.id); // tambah ke list
    });
  });
}
  ...
  #init() {
    ...
    btn.addEventListener('click', () => {
      ...
      if (overlay.classList.contains('show')) {
        ...
        }, { once: true }) // tambahkan `once` agar event tidak numpuk
      }
    })
